...
template Poseidon2Core(stateWidth, fullRounds, partialRounds) {
    // 基本参数限制（示例实现里固定为 t=3）
    assert(stateWidth == 3);
    assert(fullRounds == 8);
    assert(partialRounds == 56);

    // S-Box 指数（d=5）
    var EXP_D = 5;

    // 输入 / 输出 状态向量
    signal input state_in[stateWidth];
    signal output state_out[stateWidth];

    // 内部工作副本
    signal st[stateWidth];

    // 轮常数结构：二维数组，索引为 round -> state 索引
    var ROUND_CONSTANTS[fullRounds + partialRounds][stateWidth] = [
    ...
    // MDS 矩阵（示例值，可替换）
    var MDS_flat = [2,1,1, 1,2,1, 1,1,3]; // row-major 3x3

    // 将 flat MDS 展开并用于计算 y = M * x
    // 直接在循环中用平铺索引计算，避免额外模板调用
    function applyMDS(x) -> (y) {
        var tmp0 = 0;
        var tmp1 = 0;
        var tmp2 = 0;
        // row 0
    ...
    for (var fr = 0; fr < halfFull; fr++) {
        // add round constants
        for (var i = 0; i < stateWidth; i++) {
            st[i] <== st[i] + ROUND_CONSTANTS[roundIdx][i];
        }

        // 全分量 S-box（每个分量都做 pow5）
        for (var i = 0; i < stateWidth; i++) {
            st[i] <== pow5(st[i]);
        }

        // 线性层（显式调用 applyMDS 风格）
        var newState = applyMDS([st[0], st[1], st[2]]);
        for (var i = 0; i < stateWidth; i++) {
            st[i] <== newState[i];
        }
        roundIdx++;
    ...
    // --- 部分轮（partialRounds） ---
    for (var pr = 0; pr < partialRounds; pr++) {
        // add constants
        for (var i = 0; i < stateWidth; i++) {
            st[i] <== st[i] + ROUND_CONSTANTS[roundIdx][i];
        }

        // 仅对单一分量应用 S-box（选择 index 2）
        st[2] <== pow5(st[2]);

        // 线性层
        var newState2 = applyMDS([st[0], st[1], st[2]]);
        for (var i = 0; i < stateWidth; i++) {
            st[i] <== newState2[i];
        }
        roundIdx++;
    ...
    // --- 后半全轮 (fullRounds / 2) ---
    for (var fr2 = 0; fr2 < halfFull; fr2++) {
        for (var i = 0; i < stateWidth; i++) {
            st[i] <== st[i] + ROUND_CONSTANTS[roundIdx][i];
        }
        ...
        var newState3 = applyMDS([st[0], st[1], st[2]]);
        for (var i = 0; i < stateWidth; i++) {
            st[i] <== newState3[i];
        }
        roundIdx++;
...
/*
  顶层哈希模板：单输入 -> 单输出 示例（保留与原一致的输入/输出语义）
  - 私有输入：preimage
  - 公开输出：digest
*/
template Poseidon2HasherTop() {
    signal private input preimage;
    signal public output digest;

    var T = 3;
    var RF = 8;
    var RP = 56;

    // instantiate core permutation
    component perm = Poseidon2Core(T, RF, RP);

    // convention: state = [capacity, rate, capacity]
    perm.state_in[0] <== 0;
    perm.state_in[1] <== preimage;
    perm.state_in[2] <== 0;

    // digest 取置换输出的第 0 个分量（与原实现保持一致）
    digest <== perm.state_out[0];
}

// 导出主组件
component main { public [digest] } = Poseidon2HasherTop();
