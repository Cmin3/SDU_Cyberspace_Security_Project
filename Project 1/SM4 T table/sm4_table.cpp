...
static void sm4_key_schedule(const uint8_t MK[16], uint32_t rk[32]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        uint8_t a0 = (tmp >> 24) & 0xFF;
        uint8_t a1 = (tmp >> 16) & 0xFF;
        uint8_t a2 = (tmp >> 8) & 0xFF;
        uint8_t a3 = tmp & 0xFF;
        uint32_t t = ((uint32_t)Sbox[a0] << 24) | ((uint32_t)Sbox[a1] << 16) |
                     ((uint32_t)Sbox[a2] << 8) | Sbox[a3];
