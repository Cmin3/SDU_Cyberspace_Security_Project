#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
// SM4 固定参数
static const uint32_t FK[4] = {
    0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC
};

static const uint32_t CK[32] = {
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269,
    0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249,
    0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229,
    0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209,
    0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
};

// S盒
static const uint8_t Sbox[256] = {
    
};

// 循环左移
static uint32_t rotl(uint32_t x, uint8_t n) {
    return (x << n) | (x >> (32 - n));
}

// S盒变换
static uint32_t tau(uint32_t A) {
    uint8_t a[4];
    a[0] = (A >> 24) & 0xFF;
    a[1] = (A >> 16) & 0xFF;
    a[2] = (A >> 8) & 0xFF;
    a[3] = A & 0xFF;

    a[0] = Sbox[a[0]];
    a[1] = Sbox[a[1]];
    a[2] = Sbox[a[2]];
    a[3] = Sbox[a[3]];

    return ((uint32_t)a[0] << 24) | ((uint32_t)a[1] << 16) | ((uint32_t)a[2] << 8) | a[3];
}

// 轮函数 L 变换
static uint32_t L(uint32_t B) {
    return B ^ rotl(B, 2) ^ rotl(B, 10) ^ rotl(B, 18) ^ rotl(B, 24);
}

// 密钥扩展 L' 变换
static uint32_t L_prime(uint32_t B) {
    return B ^ rotl(B, 13) ^ rotl(B, 23);
}

// 密钥扩展
static void sm4_key_schedule(const uint8_t MK[16], uint32_t rk[32]) {
    uint32_t K[36];
    int i;
    for (i = 0; i < 4; i++) {
        K[i] = ((uint32_t)MK[4 * i] << 24) | ((uint32_t)MK[4 * i + 1] << 16) |
            ((uint32_t)MK[4 * i + 2] << 8) | MK[4 * i + 3];
        K[i] ^= FK[i];
    }
    for (i = 0; i < 32; i++) {
        uint32_t temp = K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i];
        temp = tau(temp);
        temp = L_prime(temp);
        K[i + 4] = K[i] ^ temp;
        rk[i] = K[i + 4];
    }
}

// 单块加密
static void sm4_encrypt_block(const uint8_t in[16], uint8_t out[16], const uint32_t rk[32]) {
    uint32_t X[36];
    int i;
    for (i = 0; i < 4; i++) {
        X[i] = ((uint32_t)in[4 * i] << 24) | ((uint32_t)in[4 * i + 1] << 16) |
            ((uint32_t)in[4 * i + 2] << 8) | in[4 * i + 3];
    }
    for (i = 0; i < 32; i++) {
        uint32_t temp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i];
        temp = tau(temp);
        temp = L(temp);
        X[i + 4] = X[i] ^ temp;
    }
    for (i = 0; i < 4; i++) {
        out[4 * i] = (X[35 - i] >> 24) & 0xFF;
        out[4 * i + 1] = (X[35 - i] >> 16) & 0xFF;
        out[4 * i + 2] = (X[35 - i] >> 8) & 0xFF;
        out[4 * i + 3] = X[35 - i] & 0xFF;
    }
}
// ---------- GF(2^128) 乘法 ----------
void galois_mult(const uint8_t X[16], const uint8_t H[16], uint8_t Y[16]) {
    uint8_t Z[16] = { 0 };
    uint8_t V[16];
    memcpy(V, H, 16);
    for (int i = 0; i < 128; i++) {
        int bit = (X[i / 8] >> (7 - (i % 8))) & 1;
        if (bit) {
            for (int j = 0; j < 16; j++) Z[j] ^= V[j];
        }
        int carry = V[0] & 0x80;
        for (int j = 0; j < 15; j++) V[j] = (V[j] << 1) | (V[j + 1] >> 7);
        V[15] <<= 1;
        if (carry) V[15] ^= 0x87;
    }
    memcpy(Y, Z, 16);
}

// ---------- SM4-GCM 加密 ----------
void sm4_gcm_encrypt(
    const uint8_t* key,
    const uint8_t* iv, size_t iv_len,
    const uint8_t* plaintext, size_t plen,
    uint8_t* ciphertext,
    uint8_t tag[16])
{
    uint32_t rk[32];
    sm4_key_schedule(key, rk);

    uint8_t H[16] = { 0 };
    sm4_encrypt_block(H, H, rk);

    uint8_t J0[16] = { 0 };
    if (iv_len == 12) {
        memcpy(J0, iv, 12);
        J0[15] = 1;
    }
    else {
        galois_mult(iv, H, J0);
    }

    uint8_t counter[16];
    memcpy(counter, J0, 16);

    // CTR 模式加密
    for (size_t i = 0; i < plen; i += 16) {
        uint8_t block[16];
        sm4_encrypt_block(counter, block, rk);

        size_t block_len = (i + 16 <= plen) ? 16 : plen - i;
        for (size_t j = 0; j < block_len; j++)
            ciphertext[i + j] = plaintext[i + j] ^ block[j];

        // counter++
        for (int k = 15; k >= 12; k--) {
            if (++counter[k] != 0) break;
        }
    }

    // GHASH
    uint8_t X[16] = { 0 };
    for (size_t i = 0; i < plen; i += 16) {
        uint8_t block[16] = { 0 };
        size_t block_len = (i + 16 <= plen) ? 16 : plen - i;
        memcpy(block, &ciphertext[i], block_len);
        for (int j = 0; j < 16; j++) X[j] ^= block[j];
        uint8_t tmp[16];
        galois_mult(X, H, tmp);
        memcpy(X, tmp, 16);
    }

    uint8_t E_J0[16];
    sm4_encrypt_block(J0, E_J0, rk);
    for (int i = 0; i < 16; i++) tag[i] = E_J0[i] ^ X[i];
}

// ---------- 性能测试 ----------
void performance_test(int num_tests, size_t msg_len) {
    uint8_t* plaintext = new uint8_t[msg_len];
    uint8_t* ciphertext = new uint8_t[msg_len];

    uint8_t key[16], iv[12], tag[16];

    srand((unsigned)time(NULL));
    for (size_t i = 0; i < msg_len; i++) plaintext[i] = rand() % 256;
    for (int i = 0; i < 16; i++) key[i] = rand() % 256;
    for (int i = 0; i < 12; i++) iv[i] = rand() % 256;

    clock_t start = clock();
    for (int i = 0; i < num_tests; i++) {
        sm4_gcm_encrypt(key, iv, 12, plaintext, msg_len, ciphertext, tag);
    }
    clock_t end = clock();

    double total_time_us = (double)(end - start) / CLOCKS_PER_SEC * 1e6;
    printf("消息长度: %zu 字节\n", msg_len);
    printf("加密次数: %d\n", num_tests);
    printf("总耗时: %.2f 微秒\n", total_time_us);
    printf("平均耗时: %.2f 微秒/次\n", total_time_us / num_tests);

    delete[] plaintext; // 如果用 new
    delete[] ciphertext;

}

// ---------- 测试 ----------
int main() {
    int num_tests = 1000;
    size_t msg_len = 64; // 每次加密 64 字节
    performance_test(num_tests, msg_len);
    return 0;
}